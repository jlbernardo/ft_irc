...
//   std::map<const std::string, CommandType> command_map;
//   void parse_remaining_params(std::istringstream &iss);
//   void parse_content_param(std::istringstream &iss, const std::string
//   &first_param); void parse_message_components(const std::string
//   &raw_message); void set_content_from_params(); std::string
//   add_parameters_to_message(const std::string &msg) const; std::string
//   add_content_to_message(const std::string &msg) const; bool
//   validate_command_specific() const;
...
//   // Formats message as ":nickname!username@host COMMAND target params
//   :content" std::string format_message() const;
...
//   '#'; } bool is_private_message() const { return command_type == PRIVMSG &&
//   !is_channel_message(); }
// };

// Later we might have to add:
// - parse_command()      -> Will extract IRC commands (PRIVMSG, JOIN, NICK,
// etc.)
...
// - format_irc()        -> Formats as IRC protocol message (:prefix COMMAND
// params :trailing)
